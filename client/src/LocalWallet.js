...
// user defined name from account_keys
...
const getPrivateKey = (user) => {
  if(!user) return null;
  return (ACCOUNT_KEYS.get(user).private);
}
...
const getAddress = (user) => {
  ...
  // console.log(Uint8Array.from(pubKey.slice(1)))
  const hash = keccak256(Uint8Array.from(pubKey.slice(1)));
